...
//	public static final String LOOKUPHISTORYSP = "lookupHistorySP";
	...
	public void processDataForDisplay(final String url, final String query, final String myList, final Context context) {
				...
				protected DownloadData doInBackground(String... strings) {
					if (!isCancelled()) {
						...
						switch (context.getClass().getSimpleName()) {
							...
							case "Summary":
							case "SummaryDisplay": {
								downloadData = new DownloadData(url, query, context);
								break;
							}
							case "MoveDisplay": {
								downloadData = new DownloadData(url, query, myList, context);
								break;
							}
						}
